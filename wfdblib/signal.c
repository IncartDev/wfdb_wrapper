/* file: signal.c	G. Moody	13 April 1989
            ...
            */
#pragma warning(disable:4018 4013 4244/*4996  4101 4028 4090 4700*/ 4706)
#if 0
...
static int readheader(const char *record)
{
  ...
  if (strncmp("#wfdb", linebuf, 5) == 0) { /* found the magic string */
    int j, major, minor = 0, release = 0;

    j = sscanf(linebuf + 5, "%d.%d.%d", &major, &minor, &release);
    if ((j > 0 && major > WFDB_MAJOR) ||
      (j > 1 && minor > WFDB_MINOR) ||
      (j > 2 && release > WFDB_RELEASE)) {
  ...
  if (hheader->type == WFDB_LOCAL &&
    ...
    for (r = record, qq = ss = r + strlen(r) - 1; r != ss; ss--)
      if (*ss == '/' || *ss == '\\' || *ss == ':')
        break;

    if (qq > ss && (r > ss || !(strlen(p) > 8 || strcmp(p, ss + 1) == 0))) {
  ...
  for (s = 0; s < nsig; s++) {
    ...
    if (p = strtok((char *)NULL, sep)) {
      ...
      hs->info.cksum = 0;
      //ava - в файлах базы Рохминэ получается нулевая длина
      hs->info.nsamp = ns;// (WFDB_Time)0L;
    }
...
//  [2015/Август/14 yes]
    //Нужно сохранять только название записи, а не полный путь
    char *rec = strrchr(record, '\\');
    record = rec ? rec + 1 : record;
  ...
  for (s = 0; s < nsig; s++) {
    ...
    //  [2015/Август/14 yes]
      //Нужно сохранять только название записи, а не полный путь
      char *fname = strrchr(siarray[s].fname, '\\');
      fname = fname ? fname + 1 : siarray[s].fname;

    (void)wfdb_fprintf(oheader, "%s %d", fname, siarray[s].fmt);
    if (siarray[s].spf > 1)
...
FINT setbasetime(char *string)
{
  ...
  if (btime == 0L && bdate == (WFDB_Date)0 && *string != '[' && strcmp(string,"00:00:00") != 0) {
    if (p) *(--p) = ' ';
